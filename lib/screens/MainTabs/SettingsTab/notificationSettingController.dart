...
//*********************************************************************** */
  ...
  updateNotificationSettings({bool isFromReset = false}) async {
    ...
    var response = await service.updateNotificationSettingCall(
        marketOrder: arrNotification[0]["value"],
        pendingOrder: arrNotification[1]["value"],
        executePendingOrder: arrNotification[2]["value"],
        deletePendingOrder: arrNotification[3]["value"],
        tradingSound: arrNotification[4]["value"]);
    isApiCallRunning = false;
    isResetApiCallRunning = false;
    update();
    if (response?.statusCode == 200) {
