        ...
        try {
          for (var i = 0; i < userObj.positionData!.length; i++) {
            if (socketData.data!.symbol == userObj.positionData![i].symbolName) {
              userObj.positionData![i].profitLossValue = userObj.positionData![i].tradeType!.toUpperCase() == "BUY" ? (double.parse(socketData.data!.bid.toString()) - userObj.positionData![i].price!) * userObj.positionData![i].quantity! : (userObj.positionData![i].price! - double.parse(socketData.data!.ask.toString())) * userObj.positionData![i].quantity!;
            }
          }
