...
import '../../ViewTab/MarketWatchScreen/marketWatchWrapper.dart';
import '../UserListScreen/userListController.dart';
import '../UserListScreen/userListWrapper.dart';
                                ...
                                child: Container(
                                  ...
                                  child: CustomButton(
                                    ...
                                    onPress: () {
                                      ...
                                      for (var i = 0; i < controller.arrExchange.length; i++) {
                                        controller.arrExchange[i].selectedItemsID.clear();
                                        for (var k = 0; k < controller.arrExchange[i].arrGroupList.length; k++) {
                                          for (var l = 0; l < controller.arrExchange[i].selectedItems.length; l++) {
                                            if (controller.arrExchange[i].arrGroupList[k].name == controller.arrExchange[i].selectedItems[l].name) {
                                              controller.arrExchange[i].selectedItemsID.add(controller.arrExchange[i].arrGroupList[k].groupId!);
                                            }
                                          }
                                          if (controller.arrExchange[i].arrGroupList[k].name == controller.arrExchange[i].isDropDownValueSelected.value.name && controller.arrExchange[i].isDropDownValueSelected.value.exchangeId!.isNotEmpty) {
                                            controller.arrExchange[i].selectedItemsID.add(controller.arrExchange[i].arrGroupList[k].groupId!);
                                          }
                                        }
                                        ...
                                        if (controller.arrExchange[i].isSelected == true) {
                                          ExchangeAllow arrexchangeAllow = ExchangeAllow(
                                            exchangeId: controller.arrExchange[i].exchangeId,
                                            isTurnoverWise: controller.arrExchange[i].isTurnOverSelected,
                                            isSymbolWise: controller.arrExchange[i].isSymbolSelected,
                                            groupId: controller.arrExchange[i].selectedItemsID,
                                          );

                                          controller.arrSelectedExchangeList.add(arrexchangeAllow);
                                        }
                                      }
                                      for (var i = 0; i < controller.arrExchange.length; i++) {
                                        if (controller.arrExchange[i].isHighLowTradeSelected! == true) {
                                          controller.arrHighLowBetweenTradeSelectedList.add(controller.arrExchange[i].exchangeId ?? "");
                                        }
                                      }
                                      controller.update();
                                      if (Get.arguments != null) {
                                        // controller.callForEditUser();
                                      } else {
                                        if (controller.selectedUserType.value.roleId == UserRollList.broker) {
                                          controller.callForCreateBrocker();
                                        } else if (controller.selectedUserType.value.roleId == UserRollList.master) {
                                          controller.callForCreateMaster();
                                        } else if (controller.selectedUserType.value.roleId == UserRollList.user) {
                                          controller.callForCreateUser();
                                        } else if (controller.selectedUserType.value.roleId == UserRollList.admin) {
                                          controller.callForCreateAdmin();
                                        }
                                      }
                                      controller.update();
                                    },
